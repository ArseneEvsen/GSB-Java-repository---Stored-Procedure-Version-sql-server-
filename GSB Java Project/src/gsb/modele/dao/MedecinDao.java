...
public class MedecinDao {
	...
	public static Medecin rechercher(String codeMedecin){
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select * from gsbjava.MEDECIN where CODEMED ='"+codeMedecin+"'");
		try {
			...
			System.out.println("erreur reqSelection.next() pour la requ�te - select * from gsbjava.MEDECIN where CODEMED ='"+codeMedecin+"'");
			e.printStackTrace();
			}
	...
	public static ArrayList<Medecin> retournerCollectionDesMedecins(){
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select CODEMED from gsbjava.MEDECIN");
		try{
